...
- **フロントエンド**: TypeScript
  - ランタイム: Node.js 24+（Volta で管理、ビルド時のみ）
  - パッケージマネージャー: npm
...
- **go-github**: GitHub REST API クライアント
- **Cobra**: CLI コマンドフレームワーク
- **samber/lo**: 関数型ユーティリティ
- **embed**: フロントエンドのビルド成果物を Go バイナリに埋め込み、シングルバイナリで配布

**フロントエンド (React)**:
- **React**: UI フレームワーク
- **@xyflow/react (React Flow)**: DAG（有向非巡回グラフ）の表示・編集ライブラリ。ノードのドラッグ、エッジの接続、ズーム/パン等を提供
